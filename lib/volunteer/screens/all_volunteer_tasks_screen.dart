...
class AllVolunteerTasksScreen extends StatefulWidget {
  ...
  @override
  State<AllVolunteerTasksScreen> createState() =>
      _AllVolunteerTasksScreenState();
}
    ...
    try {
      ...
      if (_searchQuery.isNotEmpty) {
        results = results.where((task) {
          ...
          final address =
              (task['pickup_address'] ?? '').toString().toLowerCase();
      ...
      results.sort((a, b) {
        switch (_sortBy) {
          ...
          case 'expiry_date':
            final aDate =
                DateTime.tryParse(a['expiry_date'] ?? '') ?? DateTime.now();
            final bDate =
                DateTime.tryParse(b['expiry_date'] ?? '') ?? DateTime.now();
            return _sortAscending
          ...
          Expanded(
            child: TabBarView(
              ...
              children: _statusFilters
                  .map((status) => _buildTasksList(status))
                  .toList(),
            ),
          ),
        ],

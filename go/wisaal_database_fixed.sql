-- ====================================================================
-- Wisaal Project: Fixed Database Script
-- ====================================================================
-- This script fixes all ON CONFLICT and syntax issues
...
CREATE TABLE ratings (
  ...
  task_id uuid REFERENCES donations(donation_id),
...
-- badges table (with UNIQUE constraint on name)
...
INSERT INTO stats (id, total_donations_completed, active_volunteers_count, active_associations_count) 
VALUES (1, 0, 0, 0) 
...
-- ====================================================================
-- Part 7: Core Business Logic Functions
-- ====================================================================
...
BEGIN
  ...
  SET is_used = true, used_by_user_id = volunteer_user_id
  WHERE id = code_id;
...
BEGIN
  ...
  WHERE donation_id = p_donation_id AND volunteer_id = current_user_id AND status = 'assigned';
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  WHERE donation_id = p_donation_id AND volunteer_id = current_user_id AND status = 'in_progress';
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Function to award points to a volunteer
...
RETURNS void AS $$
BEGIN
  UPDATE users SET points = points + p_points WHERE id = p_volunteer_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  FOR badge_record IN SELECT id FROM badges WHERE points_required <= user_points
  LOOP
...
-- ====================================================================
-- Part 9: Map and Location Functions
-- ====================================================================
...
BEGIN
  RETURN QUERY
  SELECT d.donation_id, d.title, d.status, ST_AsGeoJSON(d.location)::json
  FROM donations d
...
RETURNS void AS $$
BEGIN
  INSERT INTO user_locations (user_id, location, last_seen)
...
-- ====================================================================
-- Part 10: Volunteer and Association Management Functions
-- ====================================================================
...
BEGIN
  ...
  WHERE u.role = 'volunteer' AND u.associated_with_association_id = association_id_param
  GROUP BY u.id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  ...
  WHERE d.association_id = p_association_id AND d.scheduled_pickup_time IS NOT NULL
...
BEGIN
  ...
  IF association_id_for_donation = current_user_id THEN
    UPDATE donations SET scheduled_pickup_time = p_scheduled_time WHERE donation_id = p_donation_id;
  ELSE
...
-- دالة مشغل لإنشاء رموز QR تلقائيًا عند إ��شاء تبرع جديد
...
GRANT ALL ON TABLE user_locations TO supabase_admin;
GRANT ALL ON TABLE user_locations TO postgres;
GRANT SELECT ON TABLE user_locations TO authenticated;
